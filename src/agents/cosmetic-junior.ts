/**
 * Cosmetic Junior Agent - 화장품 실무 구현 전문가
 *
 * 배합표 작성, 보고서 생성, 데이터 변환 등 실제 구현 작업을
 * 수행하는 실무 에이전트. Oracle의 권장사항을 실행.
 *
...
export const COSMETIC_JUNIOR_PROMPT_METADATA: AgentPromptMetadata = {
  category: 'specialist',
  cost: 'CHEAP',
  promptAlias: 'CosmeticJunior',
  triggers: [
    { domain: '배합표 작성', trigger: '처방전 생성, 배합비 계산, 스케일업' },
    { domain: '보고서 생성', trigger: '안전성 보고서, 규제 서류, 검토 문서' },
    { domain: '데이터 변환', trigger: 'JSON 생성, CSV 변환, 템플릿 작성' },
  ],
  useWhen: [
    '배합표/처방전 문서 작성',
    '안전성 보고서 초안 생성',
  ...
  avoidWhen: [
    '복잡한 배합 분석 필요 (formulation-oracle 먼저)',
    '안전성 평가 필요 (safety-oracle 먼저)',
    '규제 판단 필요 (regulatory-oracle 먼저)',
    '전략적 결정 필요 (oracle 에이전트 먼저)',
  ],
};

const COSMETIC_JUNIOR_PROMPT = `<Role>
Cosmetic Junior - 화장품 실무 구현 전문가
Oracle 에이전트의 권장사항을 실제 문서로 구현하는 실무 담당자입니다.

**IDENTITY**: 실무자. 구현, 작성, 생성. 전략적 판단하지 않음.
**OUTPUT**: 배합표, 보고서, 데이터 파일. 실제 산출물.
</Role>
...
export const cosmeticJuniorAgent: AgentConfig = {
  name: 'cosmetic-junior',
  description: '화장품 실무 구현 전문가. 배합표 작성, 보고서 생성, 데이터 파일 생성 등 Oracle 권장사항의 실제 구현 담당.',
  prompt: COSMETIC_JUNIOR_PROMPT,
  tools: ['Read', 'Write', 'Edit', 'Glob', 'Grep', 'Bash'],
  model: 'sonnet',
  metadata: COSMETIC_JUNIOR_PROMPT_METADATA
};
