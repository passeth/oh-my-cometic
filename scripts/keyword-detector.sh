...
if echo "$PROMPT_LOWER" | grep -qE '\b(ultrawork|ulw)\b' || echo "$PROMPT_NO_CODE" | grep -qE '심층분석|tech dna|백서|화이트페이퍼|카피라이팅|K-Dense|k-dense' || echo "$PROMPT_LOWER" | grep -qE '\b(deep dive|comprehensive analysis)\b'; then
  cat << 'EOF'
